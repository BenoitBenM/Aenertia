...
void loop()
{
  ...
  if (millis() > loopTimer) {
    ...
    float pos1 = step1.getPositionRad();
    float pos2 = step2.getPositionRad();
    ...
    if(TILT){
      ...
      if (millis() > IMUTimer && IMUODO){
        ...
        doc2["gy"]   = g.gyro.y;
        doc2["gz"]   = g.gyro.z;
        doc2["pos1"] = pos1*3.25;  // step1.getPosition()
        doc2["pos2"] = pos2*3.25;  // step2.getPosition()
